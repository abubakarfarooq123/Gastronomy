...
class _MenuState extends State<Menu> {
                            ...
                            itemBuilder: (context, index) {
                              if (ctr.menuList.value.responseData[index]
                                ...
                                return menuCard(
                                  ...
                                  discount: ctr.menuList.value
                                      .responseData[index].discount,
                                );
                              } else {
                                return menuCard3(
                                    ctr.menuList.value.responseData[index]);
  ...
  Widget menuCard(context, List<MenuExtra> menuExtra, ResponseDatum itemName,
      {List<Variant> variatList, String imageUrl, String price, var discount}) {
    return Padding(
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        itemName.price == null && itemName.variants.length != 0
                            ? Text(
                                itemName.minPrice,
                                style: TextStyle(
                                  fontSize: 16,
  ...
  Widget menuCard3(ResponseDatum itemdata) {
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 12.0),
      child: Column(
        children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      SizedBox(
                        child: Text(
                          toBeginningOfSentenceCase(itemdata.itemName),
                          style: TextStyle(
                            fontSize: 16,
                            fontFamily: AppFont.semiBold,
                            fontWeight: FontWeight.w500,
              ...
              children: [
                ...
                GestureDetector(
                  onTap: () {
                    ...
                    cartController.extras.clear();
                    cartController.extrasPrices.clear();
                    cartController.extrasTitle.clear();
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    Padding(
                      padding:
                          const EdgeInsets.only(left: 8.0, bottom: 4, top: 8),
                      child: Text(
                        "Extras",
                        style: TextStyle(
                    ...
                    Container(
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          GestureDetector(
                            onTap: () {
                              dynamic cartExtrasList = [];
                              cartExtrasList.clear();
                              for (int i = 0;
                                  i < cartController.extras.length;
                                  i++) {
                                cartExtrasList.add({
                                  "id": cartController.extras[i],
                                  "title": cartController.extrasTitle[i],
                                  "quantity": 1
                                });
                              }
                              print(cartExtrasList.toString());
                              // ctr.cartButtonVisbility.value = true;
                              Map<String, dynamic> data = {
                                "store_id":
                                    widget.restaurantModel.id.toString(),
                                ...
                                "menu_variant_id": variantId.toString(),
                                "extras":
                                    json.encode(cartExtrasList.toString()),
                                "quantity": cartController.itemCount.toString(),
                              };
                              print(data);

                              variatList.finalPrice != null
                                  ? totalPrice.value +=
                                      (double.parse(variatList.finalPrice) *
                                              (cartController.itemCount.value)
                                                  .toDouble()) +
                                          (cartController.extrasPrices
                                                  .reduce((a, b) => a + b)) *
                                              (cartController.itemCount.value)
                                                  .toDouble()
                                  : totalPrice.value += double.parse(
                                      ...
                                      (cartController.extrasPrices
                                          ...
                                          (cartController.itemCount.value);
                              print(
                                  "value" + (variatList.finalPrice.toString()));
                              print(
                                  "extraaaaaaaaaas = ${cartController.extrasPrices.toString()}");
                              print(variatList.finalPrice != null
                                  ? " ${(double.parse(variatList.finalPrice))}   ----  ${(cartController.itemCount.value)} +   ${(cartController.extrasPrices.reduce((a, b) => a + b))}  ----------- ${(cartController.itemCount.value)}"
                                  : "${double.parse(variatList.price.toString())} ----- ${(cartController.itemCount.value)} +    ${(cartController.extrasPrices.reduce((a, b) => a + b))} ---- ${(cartController.itemCount.value)}");
                              Get.find<CartController>().cartList.add(data);
                              ...
                              //     Get.find<CartController>().cartList;
                              // print(
                              //     "final cart Data:::::::::${Get.find<CartController>().cartList}");
                              navigator.pop(context);

                              // Get.find<CartController>()
                              //     .onAddToCardButtonClick();
                              // Get.to(() => CartPage());
                            },
                            child: Container(
                              width: Get.width * 0.5,
    ...
    return SizedBox(
      ...
      child:
        ...
        () => CheckboxListTile(
            ...
            title: Container(
              child: Text(menuExtra.title,
                  style: TextStyle(
                      fontFamily: AppFont.regular,
                      color: Color(AppColor.greytextColor))),
            ),
            secondary: Obx(
              () => cartController.extras.contains(menuExtra.id)
                  ? Container(
                      margin: EdgeInsets.only(right: 15),
                      child: Text("+" + menuExtra.price.toString() + "€",
                          style: TextStyle(
                            color: Color(
                              AppColor.red,
                            ),
                  ...
                  : Container(
                      margin: EdgeInsets.only(right: 15),
                      child: Text(
                        "+" + menuExtra.price.toString() + "€",
                        style: TextStyle(
                            fontFamily: AppFont.medium, color: Colors.grey),
                      ),
                    ),
              // )
            ),
            value: cartController.extras.contains(menuExtra.id),
            checkboxShape:
                RoundedRectangleBorder(borderRadius: BorderRadius.circular(15)),
            onChanged: (v) {
              if (v) {
                cartController.extras.add(menuExtra.id);
                cartController.extrasPrices.add(menuExtra.price);
                cartController.extrasTitle.add(menuExtra.title);
                ctr.selectRadio.value = menuExtra.title;
                List abc = [];

                for (int i = 0; i < cartController.extras.length; i++) {
                  abc.add({"id": cartController.extras[i], "quantity": 1});
                }
                print("extras data:::::${abc.toString()}");
              } else {
                cartController.extras.remove(menuExtra.id);
                cartController.extrasPrices.remove(menuExtra.price);
                cartController.extrasTitle.remove(menuExtra.title);
              }
              print(v);
            }),
      ),
      ...
      //     ? Text(menuExtra.title, style: TextStyle(color: Color(AppColor.red)))
      //     : Text(
      //         menuExtra.title,
      ...
      //           "+1.50€",
      //           style:
      //               TextStyle(fontFamily: 'campton', color: Colors.grey),
      //         ),
      //   // )
      // ))
    );
